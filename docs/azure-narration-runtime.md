...
## Verified proxy synthesis — September 10, 21:23 UTC

Installed payload `903630307c2db95bec6781dceee5458cf0c456bb` in Perplexity with exit 0 and 52 packaged offline tests passing. The full local suite passes 58 tests. Proxy mode now uses system curl with `-q` (ignore curlrc), HTTPS-only transport, certificate verification, 60-second timeout, 16 MB response limit, no redirect following, and no retries. Direct Mac key mode retains Python TLS validation.

Actual remote synthesis succeeded: French Denise, 49 characters, 55,872 bytes, 4.656 seconds, MP3 mono 24 kHz/96 kbps. SHA-256 `739d73fd8b656deef6050c046e1d060ec5cedbc5c5983aa8bdad7e8b4f24452c`. This matches the already retained local private integration sample exactly. Remote ledger reports 99 characters: initial failed 49-character request, a 1-character diagnostic attempt initiated by Computer despite the no-retry instruction (the response was stopped), and the successful 49-character corrected request. No ledger reservation was erased. The earlier Python-proxy request failed; only the later curl-backed provider is verified working.

Owner purchased 2,000 Computer credits. Interim readback showed 1,788 remaining; this predates the final repair and is not the final balance. Auto-refill had become enabled (2,000 refill, 10,000 monthly limit); disabled again to preserve the owner's no-additional-charges instruction, and checked false. No credits purchased by Codex. No unattended schedule or broadcasting enabled. The Mole FM review desk reports an absent/expired account session. Full-hourly publication integration and current named exact-media reviews remain outstanding; narration connectivity is now resolved.
